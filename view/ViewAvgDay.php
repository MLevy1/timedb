...
//cycle through hrs of each day
for($x = $selSHr; $x < $selEHr; $x++) {

	//cycle through mins of each hour by selected interval
	for($y = 0; $y  < 60 ; $y+=$a) {
	
		//box#
		$z = (($x*60)+$y)/$a;

		//cycle through number of days (cols); set to 1
		for($b = 0; $b  < $Dcnt ; $b++) {
			...
			$arrsql[$b] = 
				"SELECT tblEvents.STime, tblAct.ActDesc, tblPUCodes.Color
				FROM tblEvents
...
$data[1][]  = $ind;
	...
	echo "<td  height='4pts'><font size=4pts><b>";
	echo $data[1][$z];
	echo "</font></td>";
	...
	for($c = 0; $c  < $Dcnt ; $c++) {
		...
		echo "<td width='100pts' height='25pts' bgcolor=";
		echo $ndata[$c][2][$rcnt];
		echo "><font size=2pts>";
		echo "</font></td>";
	}
