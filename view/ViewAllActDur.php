...
$sumq = 'SELECT ActID, col1, SUM(col2) AS scol FROM tblTest INNER JOIN  tblAct ON tblTest.col1=tblAct.ActDesc WHERE col1!="Untracked Time" GROUP BY col1 ORDER BY scol DESC';
...
for($x = 0; $x < ($cnt); $x++) {
	...
	echo "<tr><td>".$data1[0][$x]."</td><td class='doublecol'>".DZero($data1[5][$x]).":".DZero($data1[6][$x])."</td><td class='doublecol'>".DZero($data1[7][$x]).":".DZero($data1[8][$x])."</td><td class='doublecol'>".DZero($data1[9][$x]).":".DZero($data1[10][$x])."</td><td>".
