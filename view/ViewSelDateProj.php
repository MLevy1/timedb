...
<h1>Project Duration - Date Range</h1>
...
?>
<form method='get' action='ViewSelDateProj.php'>
...
$sql = "SELECT tblEvents.StartTime, tblEvents.STime, tblProj.ProjDesc FROM tblEvents INNER JOIN tblCont ON (tblEvents.ProID=tblCont.ContID) INNER JOIN tblProj ON (tblCont.ProjID=tblProj.ProjID)  WHERE date(tblEvents.STime) BETWEEN '$SDate' AND '$EDate' ORDER BY tblEvents.STime";

//Setup query containing the first event on the day following selected end date

$sql2 = "SELECT tblEvents.StartTime, tblEvents.STime, tblProj.ProjDesc 
FROM tblEvents 
INNER JOIN tblCont 
