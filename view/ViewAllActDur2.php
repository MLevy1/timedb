...
?>
<form method='get' action='ViewAllActDur2.php'>
...
$sql = "SELECT tblEvents.StartTime, tblEvents.STime, tblAct.ActDesc FROM tblEvents INNER JOIN tblAct ON (tblEvents.ActID=tblAct.ActID) WHERE date(tblEvents.STime) BETWEEN '$SDate' AND '$EDate' ORDER BY tblEvents.STime";

//Setup query containing the first event on the day following selected end date

$sql2 = "SELECT tblEvents.StartTime, tblEvents.STime, tblAct.ActDesc FROM tblEvents INNER JOIN tblAct ON (tblEvents.ActID=tblAct.ActID) WHERE date(tblEvents.STime) ='$EDate2' ORDER BY tblEvents.STime LIMIT 1";
