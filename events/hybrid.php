...
$QT1 = date( "Y-m-d", strtotime( "$QTime -30 day" ) );

$sql = "SELECT tblEvents.StartTime, tblEvents.STime, tblEvents.ActID, tblEvents.ProID, tblAct.ActDesc FROM tblEvents
...
$sql = "SELECT tblEvents.ActID, tblEvents.ProID, TIMESTAMPDIFF (SECOND, MAX(STime), NOW()) AS LTime FROM tblEvents INNER JOIN tblAct ON tblEvents.ActID = tblAct.ActID INNER JOIN tblCont ON tblEvents.ProID = tblCont.ContID INNER JOIN tblProj ON tblCont.ProjID = tblProj.ProjID WHERE tblCont.Active!='N' AND tblProj.ProjStatus!='Closed'  GROUP BY tblAct.ActID, tblCont.ContID ORDER BY ActID, ContID";
...
<table>
	<tr>
		...
		<td><input type="button" class="link" onclick="CheckLEvents()" value="Test" /></td>
		
		<td><input type="button" class="link" onclick="setTimeout(bclr('Orange'), 1000)" value="clr" /></td>
