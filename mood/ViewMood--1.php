...
//populate first row of reporting queries
...
date_add($dd, date_interval_create_from_date_string('1 second'));
	...
	//populates AllTimes array with the cumulative duration of -1 scored time for a given day
	
	$AllTimes[date_format($data[1][$x], 'Y-m-d')][2] = array_sum($AllTimes[date_format($data[1][$x], 'Y-m-d')][-1]);
	
	//populates AllTimes array with the cumulative duration of 0 scored time for a given day
	
	$AllTimes[date_format($data[1][$x], 'Y-m-d')][3] = array_sum($AllTimes[date_format($data[1][$x], 'Y-m-d')][0]);
	
	//populates AllTimes array with the cumulative duration of 1 scored time for a given day
	
	$AllTimes[date_format($data[1][$x], 'Y-m-d')][4] = array_sum($AllTimes[date_format($data[1][$x], 'Y-m-d')][1]);
	
	//NEW
	
	//populates AllTimes array with the cumulative duration of -0.5 scored time for a given day
	
	$AllTimes[date_format($data[1][$x], 'Y-m-d')][5] = array_sum($AllTimes[date_format($data[1][$x], 'Y-m-d')]["-0.5"]);
	
	//populates AllTimes array with the cumulative duration of 0.5 scored time for a given day
	
	$AllTimes[date_format($data[1][$x], 'Y-m-d')][6] = array_sum($AllTimes[date_format($data[1][$x], 'Y-m-d')]["0.5"]);
...
//NEW
$Reporting[8][]= round($AllTimes[$X][5],1);
$Reporting[9][]= round($AllTimes[$X][6],1);

$arrchart[7][]= round($AllTimes[$X][2],1);
$arrchart[8][]= round($AllTimes[$X][3],1);
$arrchart[9][]= round($AllTimes[$X][4],1);

//NEW
$arrchart[13][]= round($AllTimes[$X][5],1); 
$arrchart[14][]= round($AllTimes[$X][6],1);

$arrchart[10][] = date_format ($dx, 'Y');
$arrchart[11][] = date_format ($dx, 'n')-1;
$arrchart[12][] = date_format ($dx, 'j');
...
$DW[-1][0][$a][] = round($AllTimes[$X][2],1);
$DW[0][0][$a][] = round($AllTimes[$X][3],1);
$DW[1][0][$a][] = round($AllTimes[$X][4],1);
$DW["-0.5"][0][$a][] = round($AllTimes[$X][5],1);
$DW["0.5"][0][$a][] = round($AllTimes[$X][6],1);



//populates dimension 1 of the DW array with the total amount of time in each month at a mood score
$DW[-1][2][$b][] = round($AllTimes[$X][2],0);
$DW[0][2][$b][] = round($AllTimes[$X][3],0);
$DW[1][2][$b][] = round($AllTimes[$X][4],0);
$DW["-0.5"][2][$b][] = round($AllTimes[$X][5],0);
$DW["0.5"][2][$b][] = round($AllTimes[$X][6],0);
...
for( $i=0;$i<7;$i++) {
	...
	$Reporting[6][-1][]=array_sum($DW[-1][0][$i]);
...
for( $i=0;$i<7;$i++) {
	echo "<td>";
	echo array_sum($DW["-0.5"][0][$i]);
	
	$Reporting[6][0][]=array_sum($DW["-0.5"][0][$i]);
...
for( $i=0;$i<7;$i++) {
	...
	$Reporting[6][0][]=array_sum($DW[0][0][$i]);
...
for( $i=1;$i<13;$i++) {
	...
	echo round(array_sum($DW[-1][2][$i])/$MonthList[2][$i],2);
...
for( $i=1;$i<13;$i++) {
	...
	echo round(array_sum($DW["-0.5"][2][$i])/$MonthList[2][$i],2);
...
for( $i=1;$i<13;$i++) {
	...
	echo round(array_sum($DW[0][2][$i])/$MonthList[2][$i],2);
...
function drawChart1() {
	...
	var data = google.visualization.arrayToDataTable([
		...
		for( $i=0;$i<=$CTrows1;$i++) {
			...
			echo "'Date(" . $arrchart[10][$i] . ", " . $arrchart[11][$i] . ", " . $arrchart[12][$i] . ")'";
...
function drawChart2() {
	...
	var data = google.visualization.arrayToDataTable([
		...
		<?php
		echo '[';
		echo "'" . $Reporting[6][2][0]. "', ";
		echo "'" . $Reporting[6][-1][0]. "', ";
		//echo "'" . $Reporting[6][1][0]. "', ";
		echo "'" . $Reporting[6][1][0]. "'";
		echo '],';
		...
		for( $i=1;$i<7;$i++) {
			echo '[';
			echo "'" . $Reporting[6][2][$i]. "', ";
			echo $Reporting[6][-1][$i] . ", ";
			//echo $Reporting[6][1][$i] . ", ";
			echo $Reporting[6][1][$i];
			echo '],';
		}
...
function drawChart4() {
	...
	var data = google.visualization.arrayToDataTable([
		...
		<?php
		echo '[';
		echo "'" . $Reporting[7][2][0]. "', ";
		echo "'" . $Reporting[7][-1][0]. "', ";
		echo "'" . $Reporting[7][0][0]. "', ";
		echo "'" . $Reporting[7][1][0]. "'";
		echo '],';
	
		for( $i=1;$i<19;$i++) {
			echo '[';
			echo "'" . $Reporting[7][2][$i]. "', ";
			echo $Reporting[7][-1][$i] . ", ";
			echo $Reporting[7][0][$i] . ", ";
			echo $Reporting[7][1][$i];
			echo '],';
		}
